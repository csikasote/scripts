...
class Wav2Vec2EncoderLayerStableLayerNorm(nn.Module):
    def __init__(self, config):
        ...
        #    self.adapter_attn_layer = Wav2Vec2AttnAdapterLayer(config)
        #else:
        #    self.adapter_attn_layer = None
        self.layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.feed_forward = Wav2Vec2FeedForward(config)
        self.final_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)

        if getattr(config, "adapter_attn_dim", None) is not None:
            self.adapter_ffn_layer = Wav2Vec2AttnAdapterLayer(config)
        else:
            self.adapter_ffn_layer = None

        self.use_adapter_ffn_layer = config.use_adapter_ffn_layer
        self.use_adapter_attn_layer = config.use_adapter_attn_layer
    ...
    def forward(
       ...
       # if self.adapter_attn_layer is not None and self.use_adapter_attn_layer is True:
       #     hidden_states = hidden_states + self.adapter_attn_layer(hidden_states)

        hidden_states = attn_residual + hidden_states
        hidden_states = hidden_states + self.feed_forward(self.final_layer_norm(hidden_states))

        if self.adapter_ffn_layer is not None and self.use_adapter_ffn_layer is True:
            hidden_states = hidden_states + self.adapter_ffn_layer(hidden_states)
