...
class Wav2Vec2EncoderLayerStableLayerNorm(GradientCheckpointingLayer):
    def __init__(self, config):
        ...
        if getattr(config, "adapter_attn_dim", None) is not None:
            if getattr(config, "adapter_fusion", False) and getattr(config, "fusion_adapter_names", None):
                # REPLACE old fusion with dynamic attention fusion
                self.adapter_fusion_layer = Wav2Vec2AdapterFusionLayer(
                    config,
                    adapter_names=config.fusion_adapter_names,
                    temp=getattr(config, "fusion_temp", 1.0),
                    attn_dropout=getattr(config, "fusion_attn_dropout", 0.0),
                )
            else:
...
class Wav2Vec2AdapterFusionLayer(nn.Module):
    """
    Dynamic AdapterFusion: per-token attention over sub-adapter outputs.
    Runs each adapter on the same hidden_states, then attends over adapters.
    Shapes:
      hidden_states: (B, T, C)
      returns:       (B, T, C)
    """
    def __init__(self, config, adapter_names, temp: float = 1.0, attn_dropout: float = 0.0):
        super().__init__()
        self.adapter_names = list(adapter_names)
        C = config.hidden_size

        # sub-adapters (same as your existing attention adapters)
        self.adapters = nn.ModuleDict({
            name: Wav2Vec2AttnAdapterLayer(config) for name in self.adapter_names
        })

        # QKV projections for attention-over-adapters (N dimension)
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # 1) run each adapter -> list[(B,T,C)] ; stack to (N,B,T,C) -> (B,T,N,C)
        outs = [self.adapters[n](hidden_states) for n in self.adapter_names]
        V = torch.stack(outs, dim=0).transpose(0, 1).transpose(1, 2)  # (B, T, N, C)

        # 2) build Q (B,T,C), K (B,T,N,C), V (B,T,N,C)
        Q = self.q(hidden_states)   # (B,T,C)
        K = self.k(V)               # (B,T,N,C)
        V = self.v(V)               # (B,T,N,C)
